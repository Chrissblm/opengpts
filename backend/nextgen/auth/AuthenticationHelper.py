...
# import aiohttp
# from msal_extensions import TokenCache, PersistenceNotFound
...
#         self.token_cache = self._init_token_cache()

#     def _init_token_cache(self):
#         try:
#             return TokenCache.deserialize(
#                 Path(self.token_cache_path).read_text()
#             )
#         except (FileNotFoundError, PersistenceNotFound):
#             logging.warning("Token cache file not found. Initializing new token cache.")
#             return TokenCache()

#     async def acquire_token_on_behalf_of(self, user_assertion: str, scopes: list[str] = None) -> dict[str, Any]:
#         # Directly making HTTP request to the token endpoint
#         token_endpoint = f"{self.authority}/oauth2/v2.0/token"
#         headers = {'Content-Type': 'application/x-www-form-urlencoded'}
#         data = {
#             'client_id': self.server_app_id,
#             'scope': ' '.join(scopes or ["https://graph.microsoft.com/.default"]),
#             'grant_type': 'urn:ietf:params:oauth:grant-type:jwt-bearer',
#             'assertion': user_assertion,
#             'requested_token_use': 'on_behalf_of',
#             'client_secret': self.server_app_secret,
#         }

#         async with aiohttp.ClientSession() as session:
#             async with session.post(token_endpoint, headers=headers, data=data) as response:
#                 if response.status != 200:
#                     raise AuthError(await response.text(), response.status)
#                 token_response = await response.json()
#                 self._update_token_cache(token_response)
#                 return token_response

#     def _update_token_cache(self, token_response):
#         # Update the token cache with the new token response
#         self.token_cache.add(token_response)
#         # Persist the updated cache to disk
#         Path(self.token_cache_path).write_text(TokenCache.serialize(self.token_cache))
